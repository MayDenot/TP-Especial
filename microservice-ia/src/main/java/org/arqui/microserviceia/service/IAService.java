package org.arqui.microserviceia.service;

import jakarta.annotation.PostConstruct;
import jakarta.persistence.EntityManager;
import jakarta.persistence.PersistenceContext;
import jakarta.transaction.Transactional;
import org.arqui.microserviceia.client.GroqClient;
import org.arqui.microserviceia.service.dto.response.ResponseAPI;
...
public class IAService {
  ...
  @PostConstruct
  public void init() {
    this.CONTEXTO_SQL = cargarEsquemaSQL("mysql-data.sql");
  ...
  public ResponseEntity<?> procesarPrompt(String promptUsuario) {
    try {
      ...
      return new ResponseEntity<>(
              new ResponseAPI<>(false, "Error al procesar el prompt: " + e.getMessage(), null),
  ...
  public ResponseEntity<?> ejecutarSQL(String sql) {
    try {
      ...
      if (sql.trim().toUpperCase().startsWith("SELECT")) {
        ...
        data = resultados;
        return ResponseEntity.ok(new ResponseAPI<>(true, "Consulta SELECT ejecutada con éxito", data));
      } else {
        // INSERT/UPDATE/DELETE - usar executeUpdate
        int rows = entityManager.createNativeQuery(sqlToExecute).executeUpdate();
        data = rows;
        return ResponseEntity.ok(new ResponseAPI<>(true, "Sentencia DML ejecutada con éxito. Filas afectadas: " + rows, data));
      }

    } catch (Exception e) {
      log.error("Error al ejecutar SQL: {}", e.getMessage(), e);
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
              .body(new ResponseAPI<>(false, "Error al ejecutar la sentencia: " + e.getMessage(), null));
    }
  }
